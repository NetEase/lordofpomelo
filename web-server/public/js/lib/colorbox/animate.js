...
var AnimationBase = BObject.extend({
  ...
  //æ³¨æ„ï¼Œå½“è¿žç»­ä¸¤æ¬¡ä¼ è¿›æ¥çš„percentç›¸ç­‰æ—¶å€™æ˜¯ä»€ä¹ˆé€»è¾‘ï¼?
  //ç¬¬ä¸€æ¬¡è‡ªç„¶æ²¡ä»€ä¹ˆé—®é¢˜ã€?
  //ç¬¬äºŒæ¬¡ï¼šå› ä¸ºæ­¤æ—¶lastpercent == percentï¼?ä¹‹å‰çš„é€»è¾‘ä¼šé‡æ–°å¼€å§‹è®¡ç®—percentï¼ŒäºŽæ˜¯ä¼šæŠŠ[0, lastpercent]ä¹‹é—´çš„callbackå…¨éƒ¨è°ƒç”¨ä¸€éã€? å½“åœ¨sequence animationçš„æ¯ä¸ªå­animationçš„æœ«å°¾æ³¨å†Œå›žè°ƒï¼Œ
  //å› ä¸ºå­animationç¬¬ä¸€ä¸ªå¸§å¿½ç•¥ä¸è®¡æ—¶ï¼Œæ‰€ä»¥ä¼šæœ‰ä¸¤æ¬¡percentç›¸ç­‰ä¼ åˆ°sequence animationä¸­ã€‚ä½†æ˜¯é€»è¾‘ä¸Šä¸åº”è¯¥ä»Žå¤´å¼€å§‹è®¡ç®—ã€?
  //çŽ°åœ¨æ˜¯å¤šæ¬¡ä¼ åŒä¸€ä¸ªpercentåªæœ‰ç¬¬ä¸€æ¬¡ä¼šèµ·ä½œç”¨ã€?ä¹‹åŽä¸èµ·ä½œç”¨æ˜¯å› ä¸ºè¿™æ¡è¯­å¥if (lastPercent < cb1.time && cb1.time <= percent)

  //çŽ°åœ¨çš„æ”¹æ³•ï¼šcbä¼ è¿›æ¥çš„percentä¸€å®šæ˜¯å¤§äºŽç­‰äºŽä¸Šä¸€æ¬¡çš„percentï¼Œä¸Šä¸€æ¬¡percentçš„çŠ¶æ€æ¢å¤ç”±prepareï¼ˆresetï¼‰å‡½æ•°è®¾ç½®ã€?
  cb:function(percent)
  {
    if (!this._cbs)
      return;
...
exports.makeAnimationsByArray = makeAnimationsByArray;
