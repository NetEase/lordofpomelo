...
var HoverEventDecider = Decider.extend({
  ...
  {
    if (this.view().eventDecider().isMousePressed())
    {
      return;
    }
    ...
    {
      var prevMousePstn = this.view().eventDecider().mousePstn();
      evt = {type:'mouseMoved', mouseX:prevMousePstn.x, mouseY:prevMousePstn.y};
    }
  ...
  doDecideEvent:function(evt, waiters)
  {
    ...
    case 'mouseDragged':
      if (this._pressedWaiter)
      {
        ...
        //FIXME: processing js没有做类似的判断。如果完全彻底放弃processing之后，这里就不需要判断shake。
