...
Revive.prototype.update = function(){
	...
	if(this.time <= 10){
  	...
  	console.error('player revive!');
		messageService.pushMessageByAOI({route: 'onRevive', entityId : this.entity.entityId, x: this.entity.x, y: this.entity.y, hp: this.entity.hp}, {x : this.entity.x, y : this.entity.y});
		this.finished = true;
	}
	this.now = time;
};
